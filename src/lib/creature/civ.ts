...
export function buildCiv(S: Sketch, k: Kit, g: Genome, stage: Stage, ph: number, blink: boolean, citizen = 0, load?: LoadHooks) {
  ...
  if (!serpent) for (const s of [-1, 1]) {
    ...
    const w = ph + (s > 0 ? Math.PI : 0);
    const foot: V3 = [s * U * 0.03 + Math.sin(w) * U * 0.26 * gait, Math.max(0, -Math.cos(w)) * U * 0.12 * gait, s * rP * 0.62];
...
export function outfit(g: Genome, stage: Stage, citizen: number): Outfit {
  ...
  switch (stage) {
    case Stage.TRIBAL: {
      ...
      const head = choose(r, ['none', 'feather', 'headband', 'crown']);
      return {
        sleevePad: 0, pantsPad: 0,
        torso: (S, B) => {
          const len = bottom === 'loincloth' ? B.U * 0.35 : B.U * 0.7;
          skirt(S, B, bottom === 'wrapskirt' ? woven : hide, 0.9, len, bottom === 'skirt' ? 3 : 1.2);
          ...
          if (top === 'poncho') S.blob(add(B.C, [0, -B.U * 0.05, 0]), [1, 0, 0], B.rC + 3, B.rC + 3, woven, { bias: 0.1 }, B.U * 0.32);
    ...
    case Stage.MEDIEVAL: {
      ...
      const head = choose(r, ['hood', 'coif', 'straw', 'chaperon', 'none']);
    ...
    case Stage.RENAISSANCE: {
      ...
      const head = choose(r, ['beret', 'feathered', 'veil', 'none']);
      const ruff = r() < 0.65;
      ...
      return {
        ...
        torso: (S, B) => {
          ...
          if (style !== 'gown') skirt(S, B, { ...velvet, alt: velvet2.ramp, pattern: x => slash(x) }, 1.6, B.U * 0.3, 1.5);
    ...
    case Stage.INDUSTRIAL: {
      ...
      const watchChain = r() < 0.3; // a pocket-watch chain is jewellery, not equipment
      ...
      return {
        ...
        torso: (S, B) => {
          ...
          if (watchChain) S.limb(add(B.C, [B.rC * 0.95, -B.U * 0.3, -B.rC * 0.3]), add(B.C, [B.rC * 1.0, -B.U * 0.38, B.rC * 0.3]), 0.5, 0.5, metal, { bias: 0.2, noLine: true });
        },
        headFront: (S, B) => {
    ...
    case Stage.CONTEMPORARY: {
      ...
      const head = choose(r, ['none', 'cap', 'beanie', 'headphones']);
      ...
      return {
        ...
        torso: (S, B) => {
          ...
          if (bottom === 'skirt' && top !== 'dress') skirt(S, B, dye(hueOf(), 0.4, 0.35), 1.2, B.U * 0.55, 2.5);
    ...
    case Stage.SPACE: default: {
      ...
      const head = choose(r, ['circlet', 'none', 'halo', 'none']);
      return {
