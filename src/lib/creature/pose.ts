...
export class Sketch {
  ...
  blob(c: V3, axis: V3, rLong: number, rShort: number, m: Mat, o: PO = {}, rShortY = rShort) {
    rLong *= this.k; rShort *= this.k; rShortY *= this.k;
  ...
  disc(c: V3, rx: number, ry: number, rot: number, m: Mat, o: PO = {}) { rx *= this.k; ry *= this.k; const [x, y, d] = this.P(c); this.push({ s: { k: 'e', x, y, rx: Math.max(0.5, rx), ry: Math.max(0.5, ry), a: rot }, m }, d, o); }
  limb(a: V3, b: V3, ra: number, rb: number, m: Mat, o: PO = {}) {
    const A = this.P(a), B = this.P(b);
    this.push({ s: { k: 'c', x1: A[0], y1: A[1], x2: B[0], y2: B[1], r1: Math.max(0.5, ra * this.k), r2: Math.max(0.5, rb * this.k) }, m }, (A[2] + B[2]) / 2, o);
  }
